...
import (
	"github.com/carloshomar/fuudelivery/orders_api/app/handlers"
...
func SetupRoutes(app *fiber.App, sendMessageToClient func(clientID int64, message []byte) error) {
	...
	app.Get("/orders/repeat/:orderId", handlers.RepeatOrder)
	...
	app.Post("/coupons/apply", handlers.ApplyCoupon)
	...
	app.Post("/orders/schedule", handlers.ScheduleOrder)

	app.Post("/notifications/register", handlers.RegisterPushToken)

	app.Post("/loyalty/earn", handlers.EarnPoints)
	app.Post("/loyalty/redeem", handlers.RedeemPoints)
	app.Get("/loyalty/balance/:phone", handlers.GetLoyaltyBalance)
	app.Get("/loyalty/history/:phone", handlers.GetLoyaltyHistory)
	app.Get("/loyalty/calculate", handlers.CalculateLoyaltyDiscount)

	app.Post("/reviews", handlers.CreateReview)
	app.Get("/reviews/establishment/:id", handlers.GetEstablishmentReviews)
	app.Get("/reviews/product/:id", handlers.GetProductReviews)
	app.Put("/reviews/respond/:id", handlers.RespondToReview)
	app.Get("/reviews/user/:phone", handlers.GetUserReviews)
	app.Get("/reviews/rating/:establishmentId", handlers.GetEstablishmentRating)

	app.Post("/orders/pickup-code/generate", handlers.GeneratePickupCode)
	app.Post("/orders/pickup-code/validate", handlers.ValidatePickupCode)
	app.Get("/orders/pickup-code/:id", handlers.GetPickupCode)
}
