...
```
                    ...
                    │   + Certbot │
                    └──┬───┬───┬──┘
        api.          │   │   │        restaurante.   admin.
        :3000         │   │   │        :3002           :3003
   ┌──────▼─────┐ ┌───▼───▼──────┐
   │ fuudelivery│ │ 2 frontends   │  (nginx interno, estáticos)
   │    -api    │ │ (restaurante  │
   │ (monolito) │ │  e admin)     │
   └──────┬─────┘ └───────────────┘
          │
...
| ~~`payment-service`~~ | ~~`Backend/Payment/Dockerfile`~~ — **arquivado**, o diretório não existe mais. O pagamento vivo é o `payment_api`, que roda DENTRO do monolito. | — | — |
...
> **O PostgreSQL continua na nuvem** (Supabase). Este guia não cobre
> self-hosting de banco — manter na nuvem é mais seguro e barato em um VPS.
>
> **MongoDB não é mais necessário.** A consolidação para Postgres único
> terminou: nenhum caminho do monolito lê ou escreve em Mongo (verificado por
> busca em todo o código Go — as únicas referências restantes estão em
> `cmd/etl-orders` e `cmd/etl-payments`, ferramentas de migração de uma vez
> só que você não roda no VPS). Se ainda houver um cluster Atlas ativo só por
> causa deste projeto, **ele pode ser desligado** — é custo sem uso.
...
| **Go** (build dos serviços) | **1.25** (`golang:1.25-alpine`) | `Dockerfile` raiz, `Backend/*/Dockerfile`, `render.yaml` (`GO_VERSION`) |
...
a `127.0.0.1`, então **não** libere portas 3000/3002/3003/6379.
...
| `PAYMENT_API_BASE_URL` | URL pública da API de pagamento no build do WebAdmin. O `payment_api` roda DENTRO do monolito, então é o **mesmo** domínio da API. | `https://api.suaempresa.com` |
...
## 6.1 Passo 3b — Rodar as migrações no Supabase

**Faça isto ANTES de subir a stack.** O passo não existia neste guia e a
ausência dele é silenciosa: o app sobe, atende, e só falha mais tarde.

O esquema do banco é gerenciado pelos arquivos em `sql/`, aplicados em ordem
por `sql/run_all.sh`. O código também roda `AutoMigrate` do GORM na
inicialização como rede de segurança, e é por isso que a falta deste passo
não quebra na hora: o AutoMigrate **cria colunas novas**, mas não cria
**constraints, índices parciais nem CHECKs** — que é justamente onde mora a
proteção do dinheiro. Exemplos concretos do que se perde:

- `sql/20` — índice parcial único de débito por carteira. É ele que impede o
  mesmo saque ser debitado duas vezes; sem ele, a idempotência do saque passa
  a depender só do código.
- `sql/22` / `sql/23` — CHECK em `funded_by` e `discount_amount`. Sem eles,
  uma escrita direta no Postgres pode gravar um cupom bancado por "ninguém"
  ou com desconto negativo, e o split não sabe tratar.

```bash
cd ~/fuudelivery

# O script lê exatamente a MESMA variável que o backend Go usa.
# Sem pgbouncer=true aqui: o psql aplica DDL, e o pooler em modo transaction
# não é o caminho certo para isso — use a connection string DIRETA do
# Supabase (Settings → Database → Connection string → URI), não a do pooler.
export DB_CONNECTION_STRING='postgresql://postgres:SENHA@db.xxxxx.supabase.co:5432/postgres'

bash sql/run_all.sh
```

Confira o que foi aplicado — a tabela `schema_migrations` é o registro:

```bash
psql "$DB_CONNECTION_STRING" -c "SELECT version, description, applied_at
                                 FROM schema_migrations ORDER BY version::int"
```

O maior `version` listado tem de bater com o maior número em `ls sql/*.sql`.
Se faltar alguma, rode `run_all.sh` de novo: os arquivos são **idempotentes**
(`IF NOT EXISTS`, `ON CONFLICT DO NOTHING`), reaplicar é seguro.

> Repita este passo **a cada deploy que traga arquivo novo em `sql/`**. Um
> `git pull` que adiciona `sql/24` e não roda o `run_all.sh` deixa o banco
> atrás do código.
...
O `/health` do monolito detalha dependências (`postgres`, `redis`, `redis_geo`, `gateways`, ...):
...
```bash
ALLOWED_ORIGINS=https://restaurante.suaempresa.com,https://admin.suaempresa.com
```
