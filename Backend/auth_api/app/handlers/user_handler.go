...
func CreateUser(c *fiber.Ctx) error {
	...
	if tx != nil {
		...
		_, err = tx.Exec("INSERT INTO users (id, name, email, password, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())", userID, user.Name, user.Email, user.Password)
		if err != nil {
			tx.Rollback()
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
		}
